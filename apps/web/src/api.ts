...
export type PlayerProfile={xp:number;coins:number;stars:number;completedLevels:number;masteryCount:number;totalQuestions:number;correctAnswers:number;correctRate:number;studyDays:number;streak:number;masteryRate:number;weeklyActivity:Array<number>&{[index:number]:number}};
...
export const api={
  ...
  profile:()=>call<PlayerProfile>("/profile"),
