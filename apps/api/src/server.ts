...
import { idioms, levels, questions, questionsForLevel, worlds } from "./data.js";
...
function levelState(userId:string){const progress=store.getProgress(userId);return levels.map((level,index):Level=>{const saved=progress.levels.get(level.id);const previous=index===0||progress.levels.has(levels[index-1]!.id);return {...level,status:saved?"complete":previous?"active":"locked",stars:saved?.stars??0};});}
...
app.get("/health",async()=>({ok:true,service:"knowledge-expedition-api",contentVersion:"idiom-v1"}));
...
app.get("/api/v1/worlds/:worldId",async request=>{const id=(request.params as {worldId:string}).worldId;return {world:worlds.find(item=>item.id===id)??null,chapters:id==="culture"?[{id:"idiom-foundation",name:"成语初章",description:"25 个核心成语 · 65 道原创训练题"}]:[]};});
app.get("/api/v1/chapters/:chapterId",async request=>{const id=(request.params as {chapterId:string}).chapterId;if(id!=="idiom-foundation")fail("章节不存在",404);const userId=await playerId(request);return {id,name:"成语初章",knowledgeCount:idioms.length,questionCount:questions.length,levels:levelState(userId)};});
app.get("/api/v1/levels/:levelId",async request=>{const id=(request.params as {levelId:string}).levelId;const userId=await playerId(request);const level=levelState(userId).find(item=>item.id===id);if(!level)fail("关卡不存在",404);return level;});
...
app.post("/api/v1/learning-sessions",async(request,reply)=>{const parsed=startSessionInput.safeParse(request.body);if(!parsed.success)return reply.code(400).send({error:{code:"INVALID_INPUT",message:"关卡无效",requestId:request.id}});const userId=await playerId(request);const level=levelState(userId).find(item=>item.id===parsed.data.levelId);if(!level)fail("关卡不存在",404);if(level.status==="locked")fail("请先完成上一关",403);const session=store.createSession(userId,level.id);await persistPlayer(userId);return snapshot(session);});
